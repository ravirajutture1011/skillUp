...
export const getPublishedCourse = async (_, res) => {
  try {
    ...
    if (cached) {
      console.log("printing cached data ", JSON.parse(cached));
      return res.status(200).json({
    ...
    // 3. Cache for 15 minutes (900 seconds)
...
export const editCourse = async (req, res) => {
  try {
    ...
    // Also clear published list (since edits may affect visibility)
    await redis.del("published-courses");
...
export const getCourseById = async (req, res) => {
  try {
    ...
    const cached = await redis.get(cacheKey);
    if (cached) {
      console.log("printing cached getCourseById data ", JSON.parse(cached));
      return res.status(200).json({
    ...
    // 3. Cache for 30 minutes (1800 seconds)
